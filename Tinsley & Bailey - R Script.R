...
library("NCmisc")
list.functions.in.file("Analyses_Final.R")
#character(0):
...
funchir::stale_package_check('Analyses_Final.R')
#package:Matrix:
# print, summary
#package:lme4:
# glmer, lmer
...
#  R Script:
#  csIC=read.csv(file="csIC.csv",header=TRUE)
View(csIC)
...
#  3)	Lines 205-209, 260  
#Test: Focal partner identity against total number of aggressive instigations
#Model: Linear mixed-effect model (LMM) with log-likelihood ratio test (LRT)
#Result: χ25 = 1.726, N = 124, p = 0.092

#R Script:
  library(lme4)
colnames(inds)[1]<-"Focal"
ma3a1 <- lmer(X.I~Trial+(1|Focal)+(1|Interacting),data=inds)
print(ma3a1)
anova(ma3a1)
summary(ma3a1)
Reducedma3a1i <- lmer(X.I~Trial+(1|Focal),data=inds)
summary(Reducedma3a1i)
anova(Reducedma3a1i,ma3a1)
#Reducedma3a1i: X.I ~ Trial + (1 | Focal)
#ma3a1: X.I ~ Trial + (1 | Focal) + (1 | Interacting)
#              Df    AIC    BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)
#Reducedma3a1i  4 870.30 885.98 -431.15   862.30                         
#ma3a1          5 870.58 890.17 -430.29   860.58 1.7263      1     0.1889
#Interacting removed loglik:
lrt3 <- pchisq(2*(logLik(ma3a1)-logLik(Reducedma3a1i)),df=1,lower.tail=FALSE)/2
lrt3
#'log Lik.' 0.0917278 (df=5)


# 4)	Lines 205-209, 260-261 
#Test: Interacting partner identity against total number of 
#   aggressive instigations
#Model: Linear mixed-effect model (LMM) with log-likelihood ratio test (LRT)
#Result: χ25 = 0.513, N = 124, p = 0.233
...
anova(ma3a1)
summary(ma3a1)
Reducedma3a1f <- lmer(X.I~Trial+(1|Interacting),data=inds)
summary(Reducedma3a1f)
anova(Reducedma3a1f,ma3a1)
#Reducedma3a1f: X.I ~ Trial + (1 | Interacting)
#ma3a1: X.I ~ Trial + (1 | Focal) + (1 | Interacting)
#             Df    AIC    BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)
#Reducedma3a1f  4 869.09 884.77 -430.55   861.09                         
#ma3a1          5 870.58 890.17 -430.29   860.58 0.5129      1     0.4739
#Focal removed loglik:
lrt4 <- pchisq(2*(logLik(ma3a1)-logLik(Reducedma3a1f)),df=1,lower.tail=FALSE)/2
lrt4
#'log Lik.' 0.2330142 (df=5)
